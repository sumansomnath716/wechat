...
const Signin = () => {
  ...
  const validationSchema= Yup.object({
    email:Yup.string().required('Email is required').email('Not a correct a email format'),
   ...
   <>
    <form onSubmit={formik.handleSubmit} className='space-y-5'>
      <div className='block'>
        ...
        <TDInput placeholder="youremail@gmail.com" type="email" label="Email" name="email" formControlName={formik.values.email} handleChange={formik.handleChange} handleBlur={formik.handleBlur}/>
      ...
      <div className='flex flex-wrap justify-between'>
        <p >
            <input className='rounded-sm checked:bg-green-900 checked:border-green-900 focus:ring-green-900' type="checkbox"/>
            <label className="pl-3 text-sm text-gray-500 font-semibold ">Remember Me</label>
        </p>
        <p>
            <Link className="text-sm pl-5 hover:underline text-green-800 cursor-pointer" >Lost Password?</Link>
        </p>
      </div>
      <div className='block text-sm'>
        <button disabled={!formik.isValid} type="submit" className='bg-green-900 hover:bg-sky-950 w-full text-white p-3 rounded-full disabled:bg-green-900 disabled:opacity-50'>Login to your account</button>
      </div>
      </form>
       
      <div className="flex justify-center items-center">
        <p className='text-sm text-gray-500'>Not registered?</p>
        <Link to="/auth/signup" className='text-sm pl-2 text-green-800 hover:underline cursor-pointer'>Create account</Link>
      </div>
   </>
